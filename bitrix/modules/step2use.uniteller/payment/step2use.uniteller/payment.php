...
<form action="<?= ps_uniteller::$url_uniteller_pay ?>" method="post" target="_blank">
	...
	<?= $sOrderID . GetMessage('SUSP_ORDER_FROM') . $sDateInsert ?><br> <?= GetMessage('SUSP_ORDER_SUM') ?><b><?= SaleFormatCurrency($sHouldPay, $sCurrency) ?>
        ...
        // определяем "Признак способа расчета"
        $payattr = CSalePaySystemAction::GetParamValue('PAYATTR');
        // по-умолчанию - Полная предварительная оплата до момента передачи предмета расчёта
        if(!$payattr) {
            $payattr = 1;
        }
        
        // определяем "Признак предмета расчета"
        $lineattr = CSalePaySystemAction::GetParamValue('LINEATTR');
        // по-умолчанию - о реализуемом товаре, за исключением подакцизного товара (наименование и иные сведения, описывающие товар
        if(!$lineattr) {
            $lineattr = 1;
        }
